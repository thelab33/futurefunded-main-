...
This avoids iframe blocking by capturing real screenshots with Playwright,
then serving a visual board made from PNG files.

Usage:
  ffserver restart
  python scripts/visual/ff-onboarding-board.py

Optional:
  python scripts/visual/ff-onboarding-board.py --base http://127.0.0.1:5000
  python scripts/visual/ff-onboarding-board.py --no-open
"""
...
OUT = ROOT / "audit_outputs" / "visual-boards" / "onboarding"
...
def run_capture(base: str, path: str, stamp: str) -> None:
    OUT.mkdir(parents=True, exist_ok=True)

    capture_js = OUT / "capture-onboarding.mjs"
...
""".strip(),
    ...
    result = subprocess.run(
        [node, str(capture_js), base, path, str(OUT), stamp],
        cwd=str(ROOT),
        text=True,
    )
...
def write_board(base: str, page_path: str, stamp: str) -> Path:
...
<body>
  <header>
    <div class="topbar">
      <div>
        <h1>FutureFunded Onboarding Screenshot Board</h1>
  ...
  <main>
    <div class="grid">
      <section class="card desktop">
        ...
        <div class="viewport">
          <div class="shotWrap">
            <img src="./desktop.png?{stamp}" alt="Desktop onboarding screenshot">
          </div>
        </div>
      </section>
      ...
      <div class="stack">
        <section class="card tablet">
          ...
          <div class="viewport">
            <div class="shotWrap">
              <img src="./tablet.png?{stamp}" alt="Tablet onboarding screenshot">
            </div>
          </div>
        </section>
        ...
        <section class="card mobile">
          ...
          <div class="viewport">
            <div class="shotWrap">
              <img src="./mobile.png?{stamp}" alt="Mobile onboarding screenshot">
            </div>
          </div>
        </section>
      </div>
...
"""
    path_out = OUT / "onboarding-board.html"
...
def main() -> int:
    ...
    parser.add_argument("--path", default=os.environ.get("FF_BOARD_PATH", "/platform/onboarding"))
    parser.add_argument("--port", type=int, default=int(os.environ.get("FF_BOARD_PORT", "8765")))
    ...
    run_capture(args.base, args.path, stamp)
    board_file = write_board(args.base, args.path, stamp)
    ...
    print("✅ FutureFunded onboarding screenshot board ready")
