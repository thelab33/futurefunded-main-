...
const strict = process.env.FF_ONBOARDING_BOARD_STRICT === "1";
const stamp = new Date().toISOString().replace(/[:.]/g, "-");
const outRoot = path.join(root, "audit_outputs/onboarding-screenshot-board", stamp);
const latest = path.join(root, "audit_outputs/onboarding-screenshot-board/latest");
...
async function capture(browser, viewport) {
  ...
  const url = `${baseUrl}/platform/onboarding?onboarding_board=${Date.now()}-${viewport.name}`;
  const response = await page.goto(url, { waitUntil: "domcontentloaded", timeout: 60000 });
  await page.waitForLoadState("networkidle", { timeout: 12000 }).catch(() => {});
  await warmPage(page);

  const screenshotName = `onboarding-${viewport.name}-full.png`;
  ...
  const audit = await page.evaluate(() => {
    ...
    const fields = Array.from(document.querySelectorAll("input,textarea,select"))
      ...
      .map((el) => ({
        ...
        label: el.closest("label")?.innerText.trim().replace(/\s+/g, " ").slice(0, 80) || "",
      }));
    ...
    const sections = Array.from(document.querySelectorAll("main section"))
      .map((section) => {
        ...
        return {
          id: section.id || "",
          section: section.getAttribute("data-ff-onboard-section") || "",
    ...
    return {
      ...
      sections,
      contracts: {
        main: !!document.querySelector("#main-content.ffOnboardV2__main"),
        root: !!document.querySelector("[data-ff-onboard-root][data-ff-onboarding-root]"),
        flagship: !!document.querySelector("[data-ff-onboarding-flagship]"),
        header: !!document.querySelector("[data-ff-header]"),
        onboardingCss: !!document.querySelector('link[data-ff-css="onboarding.css"]'),
        onboardingJs: !!document.querySelector('script[src*="ff-onboarding.js"]'),
        campaignBasics: !!document.querySelector('[data-ff-onboard-section="campaign-basics"]'),
        brandKit: !!document.querySelector('[data-ff-onboard-section="brand-kit"]'),
        givingReadiness: !!document.querySelector('[data-ff-onboard-section="giving-readiness"]'),
        sponsorPackages: !!document.querySelector('[data-ff-onboard-section="sponsor-packages"]'),
        launchHandoff: !!document.querySelector('[data-ff-onboard-section="launch-handoff"]'),
      },
    };
  });
...
function failuresFor(result) {
  ...
  if (result.audit.fields.length < 8) failures.push(`expected at least 8 setup fields, found ${result.audit.fields.length}`);
  if (result.audit.sections.length < 5) failures.push(`expected at least 5 sections, found ${result.audit.sections.length}`);
...
function reportMd(results) {
  const lines = [
    "# FutureFunded Onboarding Screenshot Board",
    "",
...
function boardHtml(results) {
  const cards = results.map((result) => {
    ...
    const sections = result.audit.sections.map((section) => `
      <li><strong>${esc(section.section || section.id || "section")}</strong><span>${esc(section.heading || "No heading")}</span><em>${section.height}px</em></li>
    `).join("");

    const fields = result.audit.fields.slice(0, 12).map((field) => `
      <li><strong>${esc(field.name || field.type)}</strong><span>${esc(field.label || field.type)}</span></li>
    `).join("");
    ...
    return `
      <article class="card">
        ...
        <a href="${esc(result.screenshotName)}" target="_blank" rel="noreferrer">
          <img src="${esc(result.screenshotName)}" alt="FutureFunded onboarding ${esc(result.viewport.name)} screenshot">
        </a>
        <section class="facts">
        ...
        <section class="lists"><h3>Fields</h3><ul>${fields}</ul></section>
        <section class="lists"><h3>Actions</h3><ul>${actions}</ul></section>
      </article>
    `;
  }).join("");
...
<head>
  ...
  <title>FutureFunded Onboarding Screenshot Board</title>
  <style>
...
<body>
  <main class="shell">
    <section class="hero">
      <div>
        <p>FutureFunded launch workspace</p>
        <h1>Onboarding screenshot board</h1>
      </div>
      <div class="meta">${esc(baseUrl)}/platform/onboarding<br>${esc(new Date().toISOString())}</div>
    </section>
...
async function main() {
  ...
  try {
    for (const viewport of viewports) {
      console.log(`Auditing Launch onboarding / ${viewport.name} → ${baseUrl}/platform/onboarding`);
  ...
  console.log("");
  console.log("FutureFunded onboarding screenshot board complete");
  console.log("================================================");
  console.log(`Output: ${outRoot}`);
  console.log(`Latest: ${latest}`);
  console.log(`Report: ${path.join(latest, "report.md")}`);
  console.log(`Open board: python3 -m http.server 8768 --directory ${latest}`);
  console.log(`Then open:  http://127.0.0.1:8768/index.html`);
  ...
  if (failures.length) {
    ...
    console.log("");
    console.log("✅ Onboarding board passed mobile/tablet/desktop checks.");
  }
}
