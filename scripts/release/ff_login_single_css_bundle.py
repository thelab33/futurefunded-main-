...
ROOT = Path.cwd()
TEMPLATE = ROOT / "apps/web/app/templates/platform/login.html"
CSS_DIR = ROOT / "apps/web/app/static/css"
BUNDLE_NAME = "login.bundle.css"
BUNDLE_PATH = CSS_DIR / BUNDLE_NAME
PROOF_PATH = ROOT / "docs/release-proof/login-single-css-bundle-latest.json"
...
def main() -> None:
    ...
    required_sources = {"ff.css", "login.css", "ff-fortune500-final.css"}
    missing_refs = sorted(required_sources - set(source_css))
    if missing_refs:
        raise SystemExit(
            "Refusing to bundle login CSS because expected source refs are missing: "
            + ", ".join(missing_refs)
        )
    ...
    bundle_parts = [
        f"""/*
  FutureFunded Login Single CSS Bundle
  File: apps/web/app/static/css/{BUNDLE_NAME}
  Generated: {datetime.now(timezone.utc).isoformat()}

  Purpose:
  - One login stylesheet for /platform/login.
  - Preserves original source order.
  - Keeps original source files in repo for rollback/provenance.
  - Do not edit manually; regenerate with scripts/release/ff_login_single_css_bundle.py.

  Source order:
{chr(10).join(f"  - {name}" for name in source_css)}
*/

/* FF_LOGIN_SINGLE_CSS_BUNDLE_START */
"""
    ]
...
"""
    ...
    bundle_parts.append("\n/* FF_LOGIN_SINGLE_CSS_BUNDLE_END */\n")
    bundle_text = "\n".join(bundle_parts).rstrip() + "\n"
    BUNDLE_PATH.write_text(bundle_text, encoding="utf-8")

    matches = list(CSS_LINK_TAG_RE.finditer(html))
    if not matches:
        raise SystemExit("Could not find login CSS link tags to replace.")
    ...
    bundle_links = f"""
  <!-- FF_LOGIN_SINGLE_CSS_BUNDLE_SOURCES: {source_contract} -->
  <link
    rel="preload"
    as="style"
    href="{{{{ url_for('static', filename='css/{BUNDLE_NAME}') }}}}?v={{{{ _asset_v|e }}}}"
    data-ff-login-css-preload="{BUNDLE_NAME}"
    data-ff-login-bundle-sources="{source_contract}"
  />
  <link
    rel="stylesheet"
    href="{{{{ url_for('static', filename='css/{BUNDLE_NAME}') }}}}?v={{{{ _asset_v|e }}}}"
    data-ff-login-css="{BUNDLE_NAME}"
    data-ff-login-bundle-css
    data-ff-login-bundle-sources="{source_contract}"
  />
"""
    ...
    required_html = [
        BUNDLE_NAME,
        "FF_LOGIN_SINGLE_CSS_BUNDLE_SOURCES",
        "ff.css",
        "login.css",
        "ff-fortune500-final.css",
        "operator-login-v4",
    ...
    if missing_html:
        raise SystemExit("Missing required login template contracts: " + ", ".join(missing_html))
    ...
    if old_refs:
        raise SystemExit("Old login CSS href references still present: " + ", ".join(old_refs))
