...
BUNDLE_NAME = "dashboard.bundle.css"
BUNDLE_PATH = CSS_DIR / BUNDLE_NAME
PROOF_PATH = ROOT / "docs/release-proof/dashboard-single-css-bundle-latest.json"
...
def main() -> None:
    ...
    if missing_refs:
        raise SystemExit(
            "Refusing to bundle dashboard CSS because expected source refs are missing: "
            + ", ".join(missing_refs)
        )
    ...
    bundle_parts = [
        f"""/*
  FutureFunded Dashboard Single CSS Bundle
  File: apps/web/app/static/css/{BUNDLE_NAME}
  Generated: {datetime.now(timezone.utc).isoformat()}

  Purpose:
  - One operator dashboard stylesheet for /platform/dashboard.
  - Preserves original source order.
  - Keeps original source files in repo for rollback/provenance.
  - Does not alter dashboard partials, inline no-flash boot style, or JavaScript.
  - Do not edit manually; regenerate with scripts/release/ff_dashboard_single_css_bundle.py.

  Source order:
{chr(10).join(f"  - {name}" for name in source_css)}
*/

/* FF_DASHBOARD_SINGLE_CSS_BUNDLE_START */
"""
    ]
...
"""
    ...
    bundle_parts.append("\n/* FF_DASHBOARD_SINGLE_CSS_BUNDLE_END */\n")
    bundle_text = "\n".join(bundle_parts).rstrip() + "\n"
    BUNDLE_PATH.write_text(bundle_text, encoding="utf-8")

    matches = list(CSS_LINK_TAG_RE.finditer(html))
    if not matches:
        raise SystemExit("Could not find dashboard CSS link tags to replace.")
    ...
    bundle_links = f"""
  <!-- FF_DASHBOARD_SINGLE_CSS_BUNDLE_SOURCES: {source_contract} -->
  <link
    rel="preload"
    as="style"
    href="{{{{ url_for('static', filename='css/{BUNDLE_NAME}') }}}}?v={{{{ _asset_v|e }}}}"
    data-ff-dashboard-css-preload="{BUNDLE_NAME}"
    data-ff-dashboard-bundle-sources="{source_contract}"
  />
  <link
    rel="stylesheet"
    href="{{{{ url_for('static', filename='css/{BUNDLE_NAME}') }}}}?v={{{{ _asset_v|e }}}}"
    data-ff-dashboard-css="{BUNDLE_NAME}"
    data-ff-dashboard-bundle-css
    data-ff-dashboard-bundle-sources="{source_contract}"
  />
"""
    ...
    required_html = [
        BUNDLE_NAME,
        "FF_DASHBOARD_SINGLE_CSS_BUNDLE_SOURCES",
        "ff.css",
        "dashboard-modern.css",
        "ff-launch-completion.css",
        "ff-fortune500-final.css",
        ...
        "_partials/ff_dashboard_launch_assistant.html",
        "platform/_provider_readiness_panel.html",
        "platform/_sponsor_queue_preview.html",
        "platform/_sponsor_package_preview.html",
        "data-ff-dashboard-hard-noflash",
    ]

    missing_html = [item for item in required_html if item not in html]
    if missing_html:
        raise SystemExit("Missing required dashboard contracts: " + ", ".join(missing_html))
    ...
    if old_refs:
        raise SystemExit("Old dashboard CSS href references still present: " + ", ".join(old_refs))
